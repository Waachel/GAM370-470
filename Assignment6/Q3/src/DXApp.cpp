...
void DXApp::InitDemo()
{
	...
	pShaderColMultiLight->SetDirectionalLightParameters(Vect(-1, -1, 1).getNorm(),
		.1 * Vect(1, 1, 1), .5 * Vect(1, 1, 1), Vect(1, 1, 1));

	pShaderColMultiLight->SetPointLightParameters(0, Vect(0, 10, 0), 100, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));

	pShaderColMultiLight->SetPointLightParameters(1, Vect(-5, -10, 20), 100, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));

	posSL1 = Vect(0, 0, -10);
	pShaderColMultiLight->SetSpotLightParameters(1, posSL1, 4000, 1 * Vect(1, 0, 0),
		-posSL1.getNorm(), 50,
		1 * Vect(0, 0, 1), 3 * Vect(0, 0, 1), 1 * Vect(0, 0, 1, 150));


	//Terrain
	pShaderColLightTexture = new ShaderColorLightTexture(md3dDevice);
	//*
	pShaderColLightTexture->SetDirectionalLightParameters(  Vect(-1, -1, 1).getNorm(), 
												0.5 * Vect(1, 1, 1), .5 * Vect(1, 1, 1), Vect(1, 1, 1));
	//*/
	//*
	pShaderColLightTexture->SetPointLightParameters( Vect(0,10,0), 1000, .2 * Vect(0,1,0), 
											  .1 * Vect(1,1,1), 2 * Vect(1,1,1), 1*Vect(1,1,1));
	//*/
	/*
	pShaderColMultiLightTexture = new ShaderColorMultiLightTexture(md3dDevice);
	pShaderColMultiLightTexture->SetDirectionalLightParameters(Vect(-1, -1, 1).getNorm(),
		0.5 * Vect(1, 1, 1), .5 * Vect(1, 1, 1), Vect(1, 1, 1));

	pShaderColMultiLightTexture->SetPointLightParameters(0, Vect(0, 10, 0), 100, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));

	pShaderColMultiLightTexture->SetPointLightParameters(1, Vect(0, 10, 0), 100, .2 * Vect(0, 1, 0),
		.1 * Vect(1, 1, 1), 2 * Vect(1, 1, 1), 1 * Vect(1, 1, 1));

	posSL1 = Vect(0, 0, 10);
	pShaderColMultiLightTexture->SetSpotLightParameters(0, posSL1, 4000, 1.0 * Vect(1, 0, 0),
		-posSL1.getNorm(), 50,
		.2 * Vect(1, 1, 1), 3 * Vect(1, 1, 1), 1 * Vect(0, 0, 0, 150));
*/
	float scale = 30;
	...
	pModTerrain = new TerrainModel(md3dDevice, L"../Assets/Textures/heightmap.tga", 100, 10, 0, 1, 1);
	mWorldT = Matrix(SCALE, 1, 1, 1) * Matrix(TRANS, 0, -7, 0);

	pSkyBox = new SkyBox(md3dDevice, 10);
	mTex1 = new Texture(md3dDevice, L"../Assets/Textures/Skybox2.tga", D3D11_FILTER_MIN_MAG_MIP_POINT);

	pModel1 = new Model(md3dDevice, Model::PreMadeModels::UnitSphere);
	mWorld1 = Matrix(SCALE, 1, 1, 1) * Matrix(TRANS, 0, 0, 0);

	pModel2 = new Model(md3dDevice, Model::PreMadeModels::UnitSphere);
	pos2 = Vect(0, 0, -4);
	mWorld2 = Matrix(SCALE, .2f, .2f, .2f) * Matrix(TRANS, pos2);

	pModel3 = new Model(md3dDevice, Model::PreMadeModels::UnitSphere);

	mWorld3 = Matrix(SCALE, 1.5f, 1.5f, 1.5f) * Matrix(TRANS, 0, 0, 10);

	mTex2 = new Texture(md3dDevice, L"../Assets/Textures/CrateFace.tga", D3D11_FILTER_MIN_MAG_MIP_POINT);
	pModel4 = new Model(md3dDevice, Model::PreMadeModels::UnitBoxRepeatedTexture);

	mWorld4 = Matrix(SCALE, 5.0f, 5.0f, 5.0f) * Matrix(TRANS, -5, 0, 20);
...
void DXApp::DrawScene()
{
	...
	// --- Render Object #1;
	//*
	pShaderColMultiLight->SetSpotLightParameters(0, pos2, 4000, .5 * Vect(1, 0, 0),
		-pos2.getNorm(), 50,
	...
	pShaderColMultiLight->SendWorldAndMaterial(mWorld1 * Matrix(TRANS, 0, 2 * cosf(mTimer.TotalTime()), 0), Colors::Firebrick, Colors::Firebrick, Vect(1, 1, 1, 100));
	pModel1->SetToContext(md3dImmediateContext);
	pModel1->Render(md3dImmediateContext);
	pShaderColMultiLight->SendWorldAndMaterial(mWorld3 * Matrix(TRANS, 3 * cosf(mTimer.TotalTime()), 3 * cosf(mTimer.TotalTime()), 0), Colors::DarkGreen, Colors::DarkGreen, Vect(1, 1, 1, 100));
	pModel3->SetToContext(md3dImmediateContext);
	pModel3->Render(md3dImmediateContext);

	// Plane/box with light
	pShaderColLightTexture->SetSpotLightParameters(pos2, 4000, .5 * Vect(1, 0, 0),
		-pos2.getNorm(), 50,
		.2 * Vect(1, 1, 1), 3 * Vect(1, 1, 1), 1 * Vect(1, 1, 1, 150));
	//*/

	pShaderColLightTexture->SetToContext(md3dImmediateContext);
	pShaderColLightTexture->SendCamMatrices(mCam.getViewMatrix(), mCam.getProjMatrix());

	pShaderColLightTexture->SendLightParameters(eyepos);

	mTexPlane->SetToContext(md3dImmediateContext);
	pShaderColLightTexture->SendWorldAndMaterial(pModTerrain->getWorldMatrix(), Colors::Tan, 1 * Colors::Tan, Vect(.7, .7, .7, 150));

	//pPlane->Render(md3dImmediateContext);
	pModTerrain->Render(md3dImmediateContext);

	//pShaderColLightTexture->SendWorldAndMaterial(planeMat2, Colors::DarkGreen, 1 * Colors::DarkGreen, Vect(.7, .7, .7, 150));
	//pPlane->Render(md3dImmediateContext);
	...
	pShaderColLightTexture->SendLightParameters(eyepos);
	mTex2->SetToContext(md3dImmediateContext);

	pShaderColLightTexture->SendWorldAndMaterial(mWorld4 * Matrix(TRANS, 2 * cosf(mTimer.TotalTime()), 0, 0), Colors::Tan, 1 * Colors::Tan, Vect(.7, .7, .7, 150));

	pModel4->SetToContext(md3dImmediateContext);
	pModel4->Render(md3dImmediateContext);
