...
void DXApp::InitDemo()
{
	pShColorSelected = new ShaderColorSelected3DOptimized(md3dDevice);
	pShColorSelected->SetToContext(md3dImmediateContext);
	...
	mWorld4 = Matrix(SCALE, 1.5f, 1.5f, 1.5f) * Matrix(TRANS, -4, -1, -3);
...
void DXApp::DrawScene()
{
	...
	//Object 1
	mWVP = mWorld1 * mWVP;
	//pShColorSelected->SendCamMatrices(mCam.getViewMatrix(), mCam.getProjMatrix());
	pShColorSelected->SendWVPColor(mWVP, Colors::White );
