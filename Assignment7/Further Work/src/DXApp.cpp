...
void DXApp::InitDemo()
{
	...
	*/
	...
	GraphObj1 = new GraphicObject_ColorTextureLight(pModel1);
	GraphObj1->SetShader(pShaderColor, 0, mCam, Colors::Blue);
	GraphObj1->SetShader(pShaderTexture, 1, mCam, pTex1);
	GraphObj1->SetShader(pShaderColorLight, 2, mCam, Colors::Yellow);
	GraphObj1->SetShader(pShaderTextureLight, 3, mCam, pTex1);
...
void DXApp::DrawScene()
{
	...
	*/
	...
	//pShaderColorLight->SendCamMatrices(mCam.getViewMatrix(), mCam.getProjMatrix());
	GraphObj1->SetContextAndCamCTL(md3dImmediateContext, mCam.getViewMatrix(), mCam.getProjMatrix());
	GraphObj1->Render();
