...
DXApp::DXApp(HWND hwnd)
{
	...
	mpVertexShader2 = nullptr;
	mpPixelShader = nullptr;
	mpPixelShader2 = nullptr;
	mpVertexLayout = nullptr;
	mpVertexLayout2 = nullptr;
...
void DXApp::InitDemo()
{
	...
	// Compile the 2nd vertex shader
	ID3DBlob* pVSBlob2 = nullptr;
	hr = CompileShaderFromFile(L"../Assets/Shaders/SimpleColor3D.hlsl", "VS", "vs_4_0", &pVSBlob2);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
			"Problem compiling the VERTEX shader", "Error", MB_OK);
	...
	// Create the vertex shader
	hr = md3dDevice->CreateVertexShader(pVSBlob2->GetBufferPointer(), pVSBlob2->GetBufferSize(), nullptr, &mpVertexShader2);
	assert(!FAILED(hr));
	...
	// Create the input layout
	hr = md3dDevice->CreateInputLayout(layout2, numElements2, pVSBlob2->GetBufferPointer(), pVSBlob2->GetBufferSize(), &mpVertexLayout2);
	assert(!FAILED(hr));
	ReleaseAndDeleteCOMobject(pVSBlob2);
	...
	hr = CompileShaderFromFile(L"../Assets/Shaders/SimpleColor3D.hlsl", "PS", "ps_4_0", &pPSBlob2);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
			"Problem compiling the PIXEL shader", "Error", MB_OK);
	...
	// Create the pixel shader
	hr = md3dDevice->CreatePixelShader(pPSBlob2->GetBufferPointer(), pPSBlob2->GetBufferSize(), nullptr, &mpPixelShader2);
	ReleaseAndDeleteCOMobject(pPSBlob2);
	assert(!FAILED(hr));
	...
	// **** World and Color buffer
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(WorldColorData);
	bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	bd.CPUAccessFlags = 0;

	hr = md3dDevice->CreateBuffer(&bd, nullptr, &mpcbWorldColor);
	assert(SUCCEEDED(hr));


	// Connect the buffer to the render pipeline
	md3dImmediateContext->VSSetConstantBuffers(0, 1, &mpcbWVPBuffer);
	md3dImmediateContext->VSSetConstantBuffers(1, 1, &mpcbWorldColor);
	...
	//pModel4 = new Model(md3dDevice, Model::PreMadeModels::UnitBox);
	//pModel4 = pModel3;
	...
	mWorld3 = Matrix(SCALE, 1.0f, 1.0f, 1.0f) * Matrix(TRANS, 4, 2, 6);
	mWorld4 = Matrix(SCALE, 0.25f, 0.25f, 0.25f) * Matrix(TRANS, -2, 1, 3);
...
void DXApp::DrawScene()
{
	...
	wcdata.World = mWorld;
	wcdata.Color = Colors::LimeGreen;
	md3dImmediateContext->UpdateSubresource(mpcbWorldColor, 0, nullptr, &wcdata, 0, 0);
...
DXApp::~DXApp()
{
	...
	ReleaseAndDeleteCOMobject(mpVertexShader2);
	ReleaseAndDeleteCOMobject(mpPixelShader2);
	ReleaseAndDeleteCOMobject(mpVertexLayout2);
	...
	ReleaseAndDeleteCOMobject(mpcbWorldColor);
