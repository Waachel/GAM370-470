...
//	pShaderColLight->SetToContext(md3dImmediateContext);
	...
	pShaderColLight->SendWorldAndMaterial(mirrorMat, Colors::DarkGray, Colors::DarkGray, Vect(1, 1, 1, 100)); // The color is irrelevant here
	pMirror->Render(md3dImmediateContext);
...
// STEP 3:  Render objects (to render target) that should be reflected in the mirror
	...
	pShaderTexLight->SetSpotLightParameters(pos3, 4000, .5 * Vect(1, 0, 0),
		pos2.getNorm(), 50,
		.2 * Vect(1, 1, 1), 3 * Vect(1, 1, 1), 1 * Vect(1, 1, 1, 150));
...
//		   (this assumes your mirror has a tint/texture to it. If perfectly 'clear' then this isn't needed)
	...
	pShaderColLight->SendWorldAndMaterial(mirrorMat, Vect(0, .5, 1, .3f), Vect(0, .5, 1, .3f), Vect(1, 1, 1, 100)); // CAUTION!  You _must_ have alpha < 1 to have transparency
	pMirror->Render(md3dImmediateContext);
